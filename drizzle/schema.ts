...
export const modelVersions = mysqlTable("modelVersions", {
  id: int("id").autoincrement().primaryKey(), userId: int("userId").notNull(), versionName: varchar("versionName", { length: 64 }).notNull(), algorithm: mysqlEnum("algorithm", ["logistic_regression", "gaussian_nb", "lightgbm_kitnet", "abonnen_random_forest", "abonnen_gbdt"]).notNull(), featureSetJson: text("featureSetJson").notNull(), classSetJson: text("classSetJson").notNull(), annotationSetId: int("annotationSetId"), annotationSnapshotJson: text("annotationSnapshotJson"), metricsJson: text("metricsJson").notNull(), modelJson: text("modelJson").notNull(), trainedDatasetIdsJson: text("trainedDatasetIdsJson").notNull(), isActive: boolean("isActive").default(false).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(),
