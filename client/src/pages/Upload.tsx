...
import { trafficClassLabels, trafficClasses, type TrafficClass } from "@/lib/trafficClasses";
...
export default function Upload() {
  ...
  const [trafficClass, setTrafficClass] = useState<TrafficClass | "unlabeled">("unlabeled");
  ...
  const handleUpload = async (file: File) => { setProgress(0); setResult(null); setJobId(null); setStage("正在读取文件并编码上传内容"); const fileBase64 = await toBase64(file, setProgress); setStage("正在上传文件并创建解析任务"); createUpload.mutate({ name: file.name, fileBase64, trafficClass }); };
  ...
  return <><PageHeader eyebrow="样本上传" title="样本上传" description="上传离线 PCAP 后，平台将解析五元组、双向流统计、SPLT 及可见 TLS/QUIC 元数据，并将特征保存为可训练数据集。" />
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]"><div><div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-3"><div><p className="text-xs font-medium text-slate-300">初始类别</p><p className="mt-1 text-[11px] text-slate-500">为训练数据指定正常或具体恶意行为类别</p></div><Select value={trafficClass} onValueChange={value => setTrafficClass(value as typeof trafficClass)}><SelectTrigger className="w-44 border-white/10 bg-[#081421] text-slate-200"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="unlabeled">未标注集</SelectItem>{trafficClasses.map(value => <SelectItem key={value} value={value}>{trafficClassLabels[value]}</SelectItem>)}</SelectContent></Select></div><PcapDropzone onUpload={handleUpload} busy={busy} stage={activeStage} progress={job?.progress ?? progress} />
