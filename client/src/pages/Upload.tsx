...
export default function Upload() {
  ...
  useEffect(() => { if (!annotationSetId && annotationSets[0]?.id) setAnnotationSetId(annotationSets.find(set => set.id)?.id); }, [annotationSetId, annotationSets]);
  const annotationSet = annotationSets.find(set => set.id === annotationSetId) ?? annotationSets[0];
  const enabledLabels = useMemo(() => annotationSet?.labels.filter(label => label.enabled) ?? [], [annotationSet]);
  const [trafficClass, setTrafficClass] = useState<string>("unlabeled");
  useEffect(() => { if (trafficClass !== "unlabeled" && !enabledLabels.some(label => label.key === trafficClass)) setTrafficClass("unlabeled"); }, [enabledLabels, trafficClass]);
  ...
  const handleUpload = async (file: File) => { setProgress(0); setResult(null); setJobId(null); setStage("正在读取文件并编码上传内容"); const fileBase64 = await toBase64(file, setProgress); setStage("正在上传文件并创建解析任务"); createUpload.mutate({ name: file.name, fileBase64, trafficClass: trafficClass as "benign" | "c2_channel" | "data_exfiltration" | "lateral_movement" | "malware_transfer" | "unlabeled", annotationSetId: annotationSet?.id }); };
  ...
  return <><PageHeader eyebrow="样本上传" title="样本上传" description="上传离线 PCAP 后，平台将解析五元组、双向流统计、SPLT 及可见 TLS/QUIC 元数据，并将特征保存为可训练数据集。" />
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]"><div><div className="mb-4 grid gap-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-3 sm:grid-cols-2"><div><p className="text-xs font-medium text-slate-300">标注集</p><p className="mt-1 text-[11px] text-slate-500">上传后会将标注定义快照写入数据集。</p><Select value={annotationSet?.id?.toString()} onValueChange={value => setAnnotationSetId(Number(value))}><SelectTrigger className="mt-2 border-white/10 bg-[#081421] text-slate-200"><SelectValue placeholder="选择标注集" /></SelectTrigger><SelectContent>{annotationSets.map(set => <SelectItem key={set.id} value={String(set.id)}>{set.name}</SelectItem>)}</SelectContent></Select></div><div><p className="text-xs font-medium text-slate-300">初始类别</p><p className="mt-1 text-[11px] text-slate-500">从当前标注集的启用类别中选择。</p><Select value={trafficClass} onValueChange={setTrafficClass}><SelectTrigger className="mt-2 border-white/10 bg-[#081421] text-slate-200"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="unlabeled">未标注</SelectItem>{enabledLabels.map(label => <SelectItem key={label.key} value={label.key}>{label.name}</SelectItem>)}</SelectContent></Select></div></div><PcapDropzone onUpload={handleUpload} busy={busy} stage={activeStage} progress={job?.progress ?? progress} />
